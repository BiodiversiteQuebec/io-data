...
class StacItem:
	...
	def createPystacItem(self,collection, tiff_url=""):
		...
		try:
			item = stac_item.stac_create_item(str(self._cog_local_file_location), tiff_url+"/"+self._filename, self._name, self._datetime, collection, self._properties)
...
class BqIoStacPipeline:
	...
	def _pushCollectionToApi(self, collection:Collection):
		if self._api_push_func is not None:
			print("pushing Collection")
			self._api_push_func(collection)
		return

	def _pushItemToApi(self, item:StacItem):
		if self._api_push_func is not None:
			print("pushing item")
			self._api_push_func(item)
	...
	def _COG_Pystac_Push(self, collection:Collection, host:str, folder:str):
		...
		for item in collection.getItemList():
				item.getItemFile()
				item.transformToCog()
				stacitem = item.createPystacItem(collection.getCollection(), host+"/"+folder)
				self._save_COG_file(item)
				self._pushItemToApi(stacitem)
				item.deleteItemFile()
	...
	def run(self,collection:Collection, host:str, folder:str):
		...
		self._pushCollectionToApi(collection_update)
