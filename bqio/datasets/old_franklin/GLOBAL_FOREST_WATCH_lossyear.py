...
catalog = pystac.Catalog.from_file(os.path.join("/bqio/stac10/","catalog.json"))
...
collection = pystac.Collection(id=collection_id,
							   title='Global Forest Watch - Loss year',
                               description='Global Forest Watch - Loss year',
                               extent=collection_extent,
                               license='CC-BY-SA-4.0',
                               href=collection_id)

file1 = open('/bqio/datasets/lossyear.txt', 'r')
...
for file in filelist:
	try:
		...
		tif_url = upload_tiff_to_io(str(temp_output_path), filename, "GFW/lossyear")
		var=filename.replace('Hansen_GFC-2020-v1.8_lossyear','').replace('.tif','')
		properties = {
			'full_filename': filename,
			'description': 'Global Forest Watch - Loss year - '+var,
			'grid': var,
		}
...
catalog.save(dest_href='/bqio/stac11/',catalog_type=pystac.CatalogType.SELF_CONTAINED)
