...
class ThisCollection(Collection):
	...
	def createCollection(self):
		...
		collection_title = "Forest/non-forest"
		collection_description = 'Yearly time series of binary forest cover'
	...
	def createItemList(self):
		...
		for path in Path('bqio/columbia_lc/forests').rglob('*.tif'):
			...
			properties = {
				...
				'description': 'Forest cover for Colombia for year ' + year
				'year': year,
			}
			newItem: ThisStacItem = ThisStacItem(name, filename, datetime.fromisoformat(year+'-01-01'), properties, path, "raw", False)
