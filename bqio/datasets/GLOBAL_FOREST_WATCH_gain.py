...
catalog = pystac.Catalog.from_file(os.path.join("/bqio/stac11/","catalog.json"))
...
collection_id = 'gfw-gain'
collection = pystac.Collection(id=collection_id,
							   title='Global Forest Watch - Gain',
                               description='Global Forest Watch - Gain',
                               extent=collection_extent,
                               license='CC-BY-SA-4.0',
                               href=collection_id)

file1 = open('/bqio/datasets/gain.txt', 'r')
...
for file in filelist:
	try:
		...
		tif_url = upload_tiff_to_io(str(temp_output_path), filename, "GFW/gain")
		var=filename.replace('Hansen_GFC-2020-v1.8','').replace('.tif','')
		properties = {
			'full_filename': filename,
			'description': 'Global Forest Watch - Gain - '+var,
			'grid': var,
		}
