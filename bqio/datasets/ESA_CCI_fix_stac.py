...
for y in years:
	...
	uri="https://object-arbutus.cloud.computecanada.ca/bq-io/io/ESACCI-LC/"+filename
	try:
		...
		item = stac_item.stac_create_item(str(temp_input_path), uri, 'esacci-lc-'+str(y), datetime.fromisoformat(str(y)+'-01-01'),collection, properties)
...
for y in years:
	...
	item = stac_item.stac_create_item(str(path), tif_url, 'esacci-lc-'+str(y), datetime.fromisoformat(str(y)+'-01-01'),collection, properties)
	collection.add_item(item)
